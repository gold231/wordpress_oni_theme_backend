...
function gt3_content_width() {
    ...
    add_image_size('320x150', 320, 150, true);
}
...
if (!function_exists('gt3_get_theme_option')) {
    function gt3_get_theme_option($optionname, $defaultValue = null){
        $gt3_options = get_option("oni_gt3_options");
...
if(!function_exists('gt3_option')) {
    function gt3_option($name) {
        if (  class_exists( 'Redux' ) ) {
            $theme_options = get_option( 'oni' );
            if (empty($theme_options)) {
                $theme_options = get_option( 'oni_default_options' );
            }
            return isset($theme_options[$name]) ? $theme_options[$name] : null;
        }else{
            $default_option = get_option( 'oni_default_options' );
        ...
        <div id="comment-<?php comment_ID(); ?>" class="stand_comment">
            <div class="thiscommentbody">
                ...
                <div class="comment_info">
                    <div class="comment_author_says"><?php printf('%s', get_comment_author_link()) ?> <span><?php esc_html_e('says:', 'oni'); ?></span></div>
                    <div class="listing_meta">
                        <span><?php printf('%1$s', get_comment_date()) ?></span>
                        <?php edit_comment_link('<span>('.esc_html__('Edit', 'oni').')</span>', '  ', '') ?>
                    </div>
                </div>
                <div class="comment_content">
                    <?php if ($comment->comment_approved == '0') : ?>
                        <p><?php esc_html_e('Your comment is awaiting moderation.', 'oni'); ?></p>
                    <?php endif; ?>
                    <?php comment_text() ?>
                </div>
                <?php
                ...
                comment_reply_link(array_merge($args, array('depth' => $depth, 'reply_text' => '' . (($icon_post_comments)) . esc_html__('Reply', 'oni'), 'max_depth' => $max_depth_comment)))
                ?>
            </div>
        </div>
        <?php
...
if (!function_exists('gt3_page_title')) {
    function gt3_page_title(){
	    ...
	    if ( class_exists( 'WooCommerce' ) && is_product() ) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'oni');
        }elseif (is_date()) {
            $title = get_the_time('F Y');
        }elseif(is_author()){
            $title = esc_html__('Author:', 'oni') . " " . esc_html(get_the_author());
        }elseif (is_search()) {
            $title = esc_html__('Search', 'oni');
        }elseif (is_404()) {
            $title = esc_html__('404', 'oni');
        }elseif (is_archive()) {
            $title = esc_html__('Archive','oni');
...
function gt3_the_breadcrumb(){
    ...
    if(is_front_page() && !is_home()) {
        ...
        if (is_category()) {
            ...
            echo wp_kses_post($before) . esc_html__('Archive','oni').' "' . single_cat_title('', false) . '"' . wp_kses_post($after);
            ...
            echo  wp_kses_post($before) . esc_html__('Search for','oni').' "' . esc_html(get_search_query()) . '"' . wp_kses_post($after);
            ...
            echo  wp_kses_post($before) . esc_html__('Tag','oni').' "' . single_tag_title('', false) . '"' . wp_kses_post($after);

        } elseif (is_author()) {
            global $author;
            $userdata = get_userdata($author);
            echo  wp_kses_post($before) . esc_html__('Author','oni').' ' . esc_html($userdata->display_name) . wp_kses_post($after);

        } elseif (is_404()) {
            echo  wp_kses_post($before) . esc_html__('Error 404','oni') . wp_kses_post($after);
                    ...
                    echo '<div class="gt3_linear-loading-center">';
                        echo '<div class="gt3_linear-loading-center-absolute">';
                            if (!empty($preloader_logo_url)) {
                                echo '<img style="width:'.esc_attr((int)$preloader_logo_width/2).'px;height: auto;" src="'.esc_url($preloader_logo_url).'" alt="'.esc_attr__('preloader', 'oni').'">';
                            }
...
if (!function_exists('gt3_get_page_title')) {
    function gt3_get_page_title($id) {
        ...
        $gt3_page_title = is_home() && is_front_page() ? esc_html__('Blog', 'oni') : gt3_page_title();
                        ...
                        echo "<div class='container'>";
                            echo "<div class='gt3-page-title__content'>";
                                ...
                                if (is_single() && get_post_type() == 'portfolio') {
                                    ...
                                    if(count($categories)) {
                                        $item_category1 = array();
                                        foreach($categories as $category) {
                                            
                                            $item_category1[] = '<span>'.$category->name.'</span>';
                                            
                                            
                                        }
                                        $item_category = array();
                                        for ($i=0; $i < count($item_category1); $i++) { 
                                            if($item_category1[$i] != "<span>State</span>"){
                                                array_push($item_category, $item_category1[$i]);
                                            }
                                        }
                                ...
                                echo "<div class='page_title'><h1>  $item_category  ". wp_kses_post($gt3_page_title) . "</h1></div>";
                                ...
                                if (is_single() && get_post_type() == 'post') {
                                    ...
                                    echo "
                                        <div class='page_title_meta'>
                                            ...
                                            <span class='post_author'>". esc_html__('by', 'oni'). " <a href='". esc_url(get_author_posts_url($post->post_author)) ."'>". esc_html(get_the_author_meta('display_name', $post->post_author))."</a></span>
...
if (!function_exists('gt3_get_logo')) {
    function gt3_get_logo() {
        ...
        if (!empty($header_logo_src)) {
            $logo .= '<img class="default_logo" src="'.esc_url($header_logo_src).'" alt="'.esc_attr__('logo', 'oni').'"'.$logo_height_style.'>';
        }else{
        ...
        if (!empty($logo_sticky_src)) {
            $logo .= '<img class="sticky_logo" src="'.esc_url($logo_sticky_src).'" alt="'.esc_attr__('logo', 'oni').'"'.$sticky_logo_height_style.'>';
        }
        if (!empty($logo_tablet_src)) {
            $logo .= '<img class="tablet_logo" src="'.esc_url($logo_tablet_src).'" alt="'.esc_attr__('logo', 'oni').'">';
        }
        if (!empty($logo_mobile_src)) {
            $logo .= '<img class="mobile_logo" src="'.esc_url($logo_mobile_src).'" alt="'.esc_attr__('logo', 'oni').'">';
        }
        $logo .= "</a>";
        $logo .= "</div>";
        return $logo;
...
if (!function_exists('gt3_get_header_builder')) {
    function gt3_get_header_builder($id){
        ...
        if (!empty($gt3_header_builder_array)) {
            ...
            if ($gt3_header_builder_active_preset != 'default' && !empty($gt3_header_builder_active_preset) && !empty($presets[$gt3_header_builder_active_preset]) && !empty($presets[$gt3_header_builder_active_preset]['preset'])) {
            ...
            $responsive_header_over_bg .= $mobile_header_on_bg ? ' header_over_bg--mobile' : ' header_over_bg--mobile-off';
                ...
                if (!empty($gt3_header_builder_out_array['header_out'])) {
                            ...
                            if (($is_nextend_facebook || $is_nextend_google || $is_nextend_twitter) && get_option('woocommerce_enable_myaccount_registration')=='yes') {
                                ...
                                if ($is_nextend_facebook) {
                                    echo "<div class='gt3_module_button button_alignment_inline'>";
                                        ...
                                        echo '<span>'.esc_html__( 'Login with Facebook', 'oni' ).'</span>';
                                        echo "</a>";
                                    echo "</div>";
                                }
                                if ($is_nextend_google) {
                                    echo "<div class='gt3_module_button button_alignment_inline'>";
                                        ...
                                        echo '<span>'.esc_html__( 'Login with Google', 'oni' ).'</span>';
                                        echo "</a>";
                                    echo "</div>";
                                }
                                if ($is_nextend_twitter) {
                                    echo "<div class='gt3_module_button button_alignment_inline'>";
                                        echo "<a href='" . esc_url(wp_login_url() . "?loginSocial=twitter&redirect=" . get_permalink()) . "' class='button_size_normal gt3_twitter_login' title='".esc_attr__( 'Login with Twitter', 'oni' )."' data-plugin='nsl' data-action='connect' data-redirect='current' data-provider='twitter' data-popupwidth='475' data-popupheight='175'>";
                                        echo '<i class="fa fa-twitter" aria-hidden="true"></i>';
                                        echo '<span>'.esc_html__( 'Login with Twitter', 'oni' ).'</span>';
                                        echo "</a>";
                                    echo "</div>";
                                }
                                echo "</div>";
...
if (!function_exists('gt3_header_builder__container')) {
    function gt3_header_builder__container($options = null) {
                                        ...
                                        switch ($key) {
                                            ...
                                            case 'login':
	                                            ...
	                                            if ( is_user_logged_in() ) {
		                                            ...
		                                            echo '<p>' . esc_html__( 'login / register', 'oni' ) . '</p></div>';
	                                            }
	                                            echo '</div>';
                                                break;
                                            case 'cart':
                                                if ( class_exists('WooCommerce') ) {
                                                    ...
                                                    ob_start();
                                                    ?>
                                                    <a class="woo_icon" href="<?php echo wc_get_cart_url(); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'oni' ); ?>">
        ...
        $output_array = array();
        $output_array['header_out'] = !empty($gt3_header_builder__container) ? "<div class='gt3_header_builder__container'>".$gt3_header_builder__container."</div>" : '';
...
$style_formats = array(
array(
'title' => esc_html__('Font Weight', 'oni'), 'items' => array(
array('title' => esc_html__('Default', 'oni'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => 'inherit')),
array('title' => esc_html__('Lightest (100)', 'oni'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '100')),
array('title' => esc_html__('Lighter (200)', 'oni'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '200')),
array('title' => esc_html__('Light (300)', 'oni'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '300')),
array('title' => esc_html__('Normal (400)', 'oni'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '400')),
array('title' => esc_html__('Medium (500)', 'oni'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '500')),
array('title' => esc_html__('Semi-Bold (600)', 'oni'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '600')),
array('title' => esc_html__('Bold (700)', 'oni'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '700')),
array('title' => esc_html__('Bolder (800)', 'oni'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '800')),
array('title' => esc_html__('Extra Bold (900)', 'oni'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '900'))
),
),
);
