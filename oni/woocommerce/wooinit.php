...
function gt3_add_label_outofstock () {
    ...
    if (!($product->is_in_stock())) {
        echo '<div class="gt3-product-outofstock"><span class="gt3-product-outofstock__inner">'.esc_html__('Out Of Stock', 'oni').'</span></div>';
    }
}
...
function gt3_size_guide() {
    ...
    if (!empty($size_guide_url)) {
        echo '<div class="gt3_block_size_popup"><a href="#" class="image_size_popup_button theme_icon-home-repair">'.esc_html__('Size Guide', 'oni').'</a></div><!-- gt3_block_size_popup -->';
    }
}
		...
		<?php if ( $previous_product ) : ?>
            <li>
                <a href="<?php echo esc_url( $previous_product->get_permalink() ); ?>" rel="prev">
					...
					echo '</span>';
					echo '<span class="nav_text">'.esc_html__('PREV', 'oni').'</span>';
		...
		<?php if ( $next_product ) : ?>
            <li>
                <a href="<?php echo esc_url( $next_product->get_permalink() ); ?>" rel="next">
					...
					echo '</span>';
					echo '<span class="nav_text">'.esc_html__('NEXT', 'oni').'</span>';
...
function woo_add_custom_general_field() {
    ...
    woocommerce_wp_checkbox( array(
        'id'            => '_checkbox_hot',
        'label'         => esc_html__( 'Hot Product', 'oni' ),
        'description'   => esc_html__( 'Check for Hot Product', 'oni' )
    ) );
    woocommerce_wp_checkbox( array(
        'id'            => '_checkbox_new',
        'label'         => esc_html__( 'New Product', 'oni' ),
        'description'   => esc_html__( 'Check for New Product', 'oni' )
    ) );
    echo '</div>';
}
...
function gt3_hot_new_product(){
    ...
    if ( 'yes' == $is_hot ) {
        echo '<span class="onsale hot-product">'.esc_html__('Hot','oni').'</span>';
    }

    $is_new = get_post_meta( $product->get_id(), '_checkbox_new', true );
    if ( 'yes' == $is_new ) {
        echo '<span class="onsale new-product">'.esc_html__('New','oni').'</span>';
    }
}
...
function gt3_variable_choose_an_option_rename( $args ){
    ...
    if (is_object($attr)) {
        ...
        $fix = esc_html__('an option','oni');
    }
    $args['show_option_none'] = esc_html__('Choose ','oni' ).$fix;
    ...
    function gt3_search_category() {
        ...
        $form = '<div id="gt3_cat_search" class="search_cat_mobile_'.esc_attr($search_cat_exclude_mobile).'"> 
            <form action="' . get_permalink( wc_get_page_id( "shop" ) ) . '" method="GET" role="search">
                ...
                <input placeholder="'.esc_attr__('Enter your keyworld...', 'oni').'" type="text" value="'.get_search_query().'" name="s" id="s" onblur="if (this.value == \'\') {this.value = \''.esc_attr($search_cat_text_search).'\';}"  onfocus="if (this.value == \''.esc_attr($search_cat_text_search).'\') {this.value = \'\';}" />
...
function gt3_woocommerce_product_gallery_trigger(){
	echo '<div class="woocommerce-product-gallery__trigger">'.esc_html__('Fullscreen','oni').'</div>';
}
